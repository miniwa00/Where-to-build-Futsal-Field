...
colormap = LinearColormap(
    colors=["blue", "green", "yellow", "orange", "red"], vmin=0, vmax=100
)

jpath = "json/seoul_geo.json"
seoul_geo = json.load(open(jpath, encoding="utf-8"))
seoul_map = folium.Map(
    location=[37.5642135, 127.0016985], tiles="cartoDB positron", zoom_start=10.5
)
...
st.caption(
    "Made by 김윤재, 신지후, 정남주, 김종민 @ BDPV(Prof. Hyeonsil Moon) Kookmin Univ."
)
if st.button("새로고침"):
...
end_time, start_time, concatenated_df = LivingPopulationAvg(
    seoul_map, seoul_geo
).livingPopulationAvg()
